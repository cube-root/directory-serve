...
const directory = (req, res, { path, uploadFile = true } = {}) => {
  const currentPath = parseUrl(req).pathname;
  const stream = send(req, currentPath, { index: false, root: path, dotfiles: 'allow' });
  ...
  stream.on('directory', async (resp, dirPath) => {
    ...
    await new Promise((resolve) => {
      fs.readdir(directoryPath, (err, files) => {
        ...
        let directoryUp = currentPath.split('/').filter(path => path);
        directoryUp.splice(-1)
        directoryUp = '/'+directoryUp.join('/');
        
        htmlResponse
          += '<section class="bg-blue-50 py-6 px-4 flex-1 overflow-y-auto">'
                +'<h3 class="text-xl font-sans font-medium mb-4 text-slate-500">Directory List</h3>'
                +'<a target="_self"' 
                +'class="text-blue-600 text-sm px-3 py-1 bg-blue-100 rounded-full inline-flex space-x-1"' 
                +`href="${directoryUp}"> 

                <svg style="pointer-events: none;" class="w-4 h-4" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        files.forEach((file) => {
          ...
          if (fs.lstatSync(filePath).isFile()) {
            htmlResponse
              ...
              + `<a onclick="nameClick(event, this)" class="text-blue-800 text-sm px-2 py-1 bg-blue-200 rounded-full inline-flex space-x-1" href="${file}?rename=true"> 
                  <span>${file}</span>  
                  <input onkeydown="renameInput(event, this)" type="text" style="display: none;" value="${file}" /> 
                  <button class="inline-flex" class="w-4 h-4">
              ...
              + '<span>Download</span>'
              + '</a>'             
              + `<a target="_self" class="text-blue-600 text-sm px-3 py-1 bg-blue-100 rounded-full inline-flex space-x-1" href="${file+'?delete=true'}">`
